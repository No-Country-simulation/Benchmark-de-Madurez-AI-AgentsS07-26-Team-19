...
import  ResultsProcessing from '@/components/results/ResultsProcessing'

type ProcessingStage = 1 | 2 | 3 | 4

const orderBy = (
  a: { order: number; id: number },
  b: { order: number; id: number },
...
export default function Diagnostic() {
  ...
  // Estados de la pantalla de procesamiento
  const [isProcessing, setIsProcessing] = useState(false)
  const [processingStage, setProcessingStage] =
    useState<ProcessingStage>(1)
  ...
  const answeredCount = sortedQuestions.filter(
    (q) => storeAnswers[q.id] != null,
  ).length

  const isComplete = sortedQuestions.every(
    (q) => storeAnswers[q.id] != null,
  )

  const canGoNext =
    currentQuestion != null &&
    storeAnswers[currentQuestion.id] != null

  if (isPending) {
    return (
      <p className="min-h-screen bg-[#020D1B] py-16 text-center text-white">
        Cargando preguntas…
      </p>
    )
  }
  ...
  if (isError || sortedQuestions.length === 0) {
    return (
      <p className="min-h-screen bg-[#020D1B] py-16 text-center text-red-600">
        No se pudieron cargar las preguntas. Verificá que la API esté
        disponible.
      </p>
    )
  }
  ...
  function goToNext() {
    ...
    setCurrentIndex((index) =>
      Math.min(index + 1, sortedQuestions.length - 1),
    )
  }
  ...
  async function handleSubmit() {
    if (!isComplete) {
      setSubmitError(
        'Respondé todas las preguntas antes de ver los resultados.',
      )
      return
    }
    ...
    const payload: DiagnosticAnswer[] = sortedQuestions.map(
      (question) => ({
        question_id: question.id,
        value: ANSWER_VALUE_SCHEMA.parse(
          storeAnswers[question.id],
        ) as DiagnosticAnswer['value'],
      }),
    )
    ...
    try {
      ...
      setSubmitError(
        error instanceof Error
          ? error.message
          : 'Ocurrió un error al enviar el diagnóstico.',
      )
    }
  }
